...
description: Deploy a complete self-hosted InterAct installation with separate Supabase, Gemini API, GitHub Pages, Reurl.cc, OpenAI Realtime, and Windows portable-app configuration. Use when setting up InterAct for a new instructor, replacing service credentials, cloning or forking InterAct for private testing, diagnosing an incomplete deployment, or rebuilding InterAct.exe / InterActPlus.exe for another person's infrastructure.
---
...
InterAct ships as two Windows apps built from the same source:

- `InterAct.exe` (standard) — screenshot interaction, danmaku, polls/quizzes, lottery, buzzer, AI session report. No live captions or interpretation.
- `InterActPlus.exe` (plus) — everything above, plus live captions and real-time interpretation audio via OpenAI Realtime.

Ask which edition(s) the instructor needs before starting. The standard edition skips the OpenAI phase entirely.
...
- Reurl.cc API key.
- OpenAI project API key (Plus edition only).

Treat the Supabase publishable key and URLs as public configuration. Treat Gemini, Reurl, OpenAI, Supabase secret keys, service-role keys, database passwords, and GitHub tokens as secrets.
...
5. Plus edition only: read [references/openai.md](references/openai.md), then run `scripts/deploy-openai.ps1`.
6. Read [references/windows.md](references/windows.md), then run `scripts/package-windows.ps1` with `-Edition standard`, `-Edition plus`, or both. Confirm the resulting root exe opens and its QR code targets the new GitHub Pages site.
...
- Never expose `GEMINI_API_KEY`, `REURL_API_KEY`, or `OPENAI_API_KEY` through a `VITE_` variable.
...
Run `pnpm lint` and `pnpm build`. Test with two devices on different networks: create a session, join by QR code, submit a message, answer a screenshot question, run AI analysis, test Reurl, draw a learner, run the buzzer, send an Exit Ticket, and export the report. For the Plus edition, also start live captions and confirm a participant can select and hear an interpretation language.
