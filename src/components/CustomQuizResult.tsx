...
export function CustomQuizResult({ question, results, onlineCount }: Props) {
  if (!results) {
      ...
      <div className="quiz-attempt-list">
        {results.attempts.map((attempt) => (
          <article key={attempt.id}>
            <div><strong>{attempt.participant_name}</strong><span>{attempt.status === 'graded' ? `${attempt.total_score}/${attempt.max_score}` : attempt.status === 'failed' ? '評分失敗' : '評分中'}</span></div>
